...
gen windfall_rel = windfall / l.milex
winsor2 windfall_rel, cuts(0.5 99.5)
reghdfe lmilex5 windfall_rel_w, absorb(iso year) cluster(iso year) nocons

//// DO IT IN CHANGES
predict double milex_chg_pred, xb
replace milex_chg_pred = (milex_chg_pred) * l.milex
...
// First export endogenous regression
eststo clear
label var TMA "TMA"
eststo: reghdfe milex TMA, noabsorb cluster(iso year)

eststo: reghdfe milex TMA, absorb(iso) cluster(iso year)
estadd local hascfe "\checkmark"

eststo: reghdfe milex TMA, absorb(iso year) cluster(iso year)
estadd local hascfe "\checkmark"
estadd local hasyfe "\checkmark"

esttab using "${DIR_DATA_EXPORTS}/reg_endogenous.tex", star(* 0.1 ** 0.05  *** 0.01) keep(TMA) r2 label fragment se tex nomtitles nonumbers replace  stats(hascfe hasyfe r2 N, fmt(1 1 3 "%9.0fc") label("Country fixed effects" "Year fixed effects" "\$R^2\$" "\$N\$"))
eststo clear
...
eststo: xtscc milex3 l(0/2).ALLY_pred_change l(0/2).windfall l(1/2).milex0 l(0/2).conflict_high l(0/2).conflict_low i.year, fe
estadd local hascfe "\checkmark"
estadd local hasyfe "\checkmark"
estadd local hasctrl "\checkmark"
...
forvalues h=0/`h_max' {
	xtscc milex`h' l(0/3).ALLY_pred_change l(0/3).windfall l(1/3).milex0 l(1/3).d.gdp i.year, fe level(90)
	...
	xtscc milex`h' l(0/3).TMA l(0/3).windfall l(1/3).milex0 l(1/3).d.gdp i.year, fe level(90)
...
twoway ///
	...
	, yline(0) xtitle("Year after ally spending shock") ytitle("Military expenditures (in USD)") title("Response to 100 USD increase of allied military spending") legend(off) scale(1.4) ysize(9) xsize(20) xla(0(1)12)
...
twoway ///
	...
	, yline(0) xtitle("Year after ally spending shock") ytitle("Military expenditures (in USD)") title("Response to endog 100 USD increase of allied military spending") legend(off) scale(1.4) ysize(9) xsize(20) xla(0(1)12)
...
forvalues h = 0/`h_max' {	
	...
	while (`i' < `=maxit') {
		...
		* [rebuild any regressors that depend on beta here, e.g., RGP(beta)]
		...
		xtscc cmilex`h' l(0/3).ALLY_pred_change_pos l(0/3).term l(0/3).windfall l(1/3).milex0 l(1/3).d.gdp i.year, fe level(90)
...
twoway ///
	...
	, yline(0) legend(off) scale(1.4) ysize(9) xsize(20) xla(0(1)12)
