...
* 6. Overwrite with Hannes' countries where available
*merge 1:1 iso year using "${DIR_DATA_PROCESSED}/common/budgets.dta", nogen keep(master matched using)
*merge 1:1 iso year using "${DIR_DATA_PROCESSED}/common/gdp.dta", nogen keep(master matched)
*replace milex = . if country_hannes == 1
*replace milex = milratio_hannes * gdp if country_hannes == 1
*drop *_hannes
*drop gdp


* 7. Save final dataset
save "${DIR_DATA_PROCESSED}/common/milex.dta", replace
