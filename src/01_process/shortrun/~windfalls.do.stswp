...
drop if global_market_share == .

xtset gid year
gen market_share_drop = global_market_share - l.global_market_share 
...
egen gid = group(iso cmdcode)
xtset gid year
gen windfall = (export_value - import_value) * ret
...
save "${DIR_DATA_PROCESSED}/shortrun/windfalls_disaggregated.dta", replace
...
save "${DIR_DATA_PROCESSED}/shortrun/windfalls.dta", replace
